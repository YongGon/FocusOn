		...
		if (getSdkVersion() >= 5)
		{
			try {
				result = (IBCScaleGestureDetector)getClass().getClassLoader().
					loadClass("com.one.android.Bc.ScaleGestureDetector").
			...
			synchronized (this)
			{
				if (bcStorageContext == null)
				{
					if (getSdkVersion() >= 8)
					{
						try
						{
							bcStorageContext = (IBCStorageContext)getClass().getClassLoader().loadClass("com.one.android.Bc.BCStorageContext8").newInstance();
						}
						catch (Exception ie)
						{
					...
					else
					{
						try
						{
							bcStorageContext = (IBCStorageContext)getClass().getClassLoader().loadClass("com.one.android.Bc.BCStorageContext7").newInstance();
						}
						catch (Exception ie)
						{
							throw new RuntimeException("Error instantiating", ie);
